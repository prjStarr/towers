...
namespace Assets
{
    ...
    class WorkerAuthorityVisualizer : MonoBehaviour, MaterialChangeListener
    {
        [Require] private WorkerAuthorityReader authority;
        ...
        public void OnMaterialsChange(Renderer ren, Material[] mats)
        {
            SetColourInMaterials(mats, GetColour(authority.Id));
        }
        ...
        void Awake()
        {
            ...
            var changer = GetComponent<ChangeMaterial>();
            if(changer!=null)
            {
                changer.AddListener(this);
            }
        }

        public void OnEnable()
        {
            authority.IdUpdated += SetColour;
        }


        public Color nextColor()
        {
            var hsv = new HSBColor(hue, saturation, brightness);
            var color = HSBColor.ToColor(hsv);
            hue += 0.61803398875f;
            if (hue > 1)
            {
                hue -= 1;
        ...
        private void SetColourInMaterials(Material[] mats, Color col)
        {
            foreach (var mat in mats)
            {
                mat.SetColor("_WorkerColor", col);
            }
        }
        ...
        private Color GetColour(int id)
        {
            ...
            {
                colors.Add(id, nextColor());
            }
            return colors[id];
        }

        private void SetColour(int id)
        {
            var colour = GetColour(id);

            foreach (var ren in renderers)
            {
                SetColourInMaterials(ren.materials, colour);
            }
        }
    }
}
